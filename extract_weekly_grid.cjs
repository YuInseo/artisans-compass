...
// The merged sessions logic
const getMergedSessionsContent = weeklyViewContent.match(/const getMergedSessionsForDay = \(day: Date\) => \{[\s\S]*?return finalBlocks;\n    };/)[0];

const useWeeklyMergedSessionsPath = path.join(__dirname, 'src/components/dashboard/hooks/useWeeklyMergedSessions.ts');
const useWeeklyMergedSessionsContent = `import { isSameDay, addMinutes, differenceInSeconds } from 'date-fns';
...
export function useWeeklyMergedSessions({
    ...
    ${getMergedSessionsContent.split('\n').map(line => '    ' + line).join('\n').replace('const getMergedSessionsForDay = (day: Date) => {', 'const getMergedSessionsForDay = (day: Date) => {')}

    return { getMergedSessionsForDay };
}
`;

fs.writeFileSync(useWeeklyMergedSessionsPath, useWeeklyMergedSessionsContent);

// The grid JSX block goes from `<div className="w-14 shrink-0` up to `{days.map(day => (` till the end of `</div>))} </div></div>`
// Let's create `WeeklyGrid.tsx`
const weeklyGridPath = path.join(__dirname, 'src/components/dashboard/weekly/WeeklyGrid.tsx');

const gridContent = `import { isSameDay, addMinutes, format } from 'date-fns';
...
fs.writeFileSync(weeklyGridPath, gridContent);

console.log("Successfully generated useWeeklyMergedSessions and WeeklyGrid");
