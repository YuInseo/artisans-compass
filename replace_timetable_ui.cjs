...
export function TimeTableGraph({
    ...
    const { sessionBlocks, TOTAL_MINUTES, TOTAL_HOURS } = useTimeTableData(
        sessions, date, liveSession, allSessions, projects, now, renderMode, plannedSessions, nightTimeStart, settings
    );
...
fs.writeFileSync('src/components/dashboard/TimeTableGraph.tsx', code);
console.log('Successfully replaced TimeTableGraph.tsx');
